...
A collection of simple Lua and Rust games, and an API for playing them including an English dictionary (for word puzzles), websocket multiplayer, automatic state saving including undo/redo, and state sharing via URL. You can also upload your own Lua games and play in the public web version.
...
The web version stores game states in [`localStorage`](https://developer.mozilla.org/en-US/docs/Web/API/Window/localStorage), so can play on mobile and not lose your progress if you briefly put your phone away.
...
* _(Experimental)_ **In-browser Lua editor**: Edit the Lua games' code right in the browser, and press a button to reload the game with your edits. Check "Options" menu for the "Open in-browser editor" button.
