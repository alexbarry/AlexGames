...
use crate::libs::swipe_tracker::{CursorEvt, CursorEvtType, SwipeEvt};

use crate::gem_match::gem_match_core::{Pt, State};
use crate::gem_match::gem_match_draw::{cell_size, GemMatchDraw, FPS};
use crate::gem_match::gem_match_serialize::{deserialize, serialize};
...
impl AlexGamesGemMatch {
    fn handle_swipe(&mut self, evt: SwipeEvt) {
        ...
        if let Ok(move_result) = move_result {
            ...
            self.draw
                .handle_swipe_bad_move(evt.pos, evt.dir, &self.state);
        }
    }
    ...
    fn save_state(&self) {
        if let Ok(serialized_state) = serialize(self.state) {
            self.callbacks
                .save_state(self.session_id.unwrap(), serialized_state);
        }
    }
}
...
impl AlexGamesApi for AlexGamesGemMatch {
    ...
    fn handle_touch_evt(&mut self, evt_id: &str, touches: Vec<rust_game_api::TouchInfo>) {
        let cursor_evt_type = match evt_id {
            ...
            _ => {
                panic!("unhandled touch evt {}", evt_id);
            }
        };
    ...
    fn start_game(&mut self, session_id_and_state: Option<(i32, Vec<u8>)>) {
        if let Some((session_id, serialized_state)) = session_id_and_state {
            if let Ok(state) = deserialize(&serialized_state) {
                ...
                self.callbacks
                    .set_status_err("Error decoding serialized state");
            }
