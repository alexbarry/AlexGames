...
use crate::gem_match::gem_match_core::{GemChanges, GemType, Pt, State, BOARD_HEIGHT, BOARD_WIDTH};
...
impl StateAnimation {
    ...
    fn add_animation(&mut self, anim: GemAnimation) {
        self.gem_pos_adjustments_by_pos
            .get_mut(&anim.src_cell)
            .expect("empty?")
            .push(anim);
    }
}
...
impl GemAnimation {
    fn is_current(&self, time_ms: TimeMs) -> bool {
        return self.start_time_ms <= time_ms && time_ms < self.start_time_ms + self.total_time_ms;
    }
}
...
impl GemMatchDraw {
    ...
    fn draw_gem(&self, gem_type: GemType, circ_y: i32, circ_x: i32, alpha: f64, piece_radius: i32) {
        ...
        let (colour, outline_colour) = match gem_type {
            GemType::SAPPHIRE => match is_dark_mode {
                false => ("#0f52ba", "#000088"),
                true => ("#0f228a", "#000058"),
            },
            //GemType::EMERALD  => ("#50c878", "#008800" ),
            GemType::EMERALD => match is_dark_mode {
                false => ("#30c858", "#008800"),
                true => ("#009828", "#005800"),
            },
            GemType::RUBY => match is_dark_mode {
                false => ("#9b111e", "#440000"),
                true => ("#6b010e", "#140000"),
            },
            GemType::AMETHYST => match is_dark_mode {
                false => ("#9966cc", "#440044"),
                true => ("#69369c", "#140014"),
            },
            GemType::TOPAZ => match is_dark_mode {
                false => ("#ffd700", "#888866"),
                true => ("#af7700", "#585866"),
            },
        ...
        match gem_type {
            ...
            GemType::EMERALD => {
                ...
                let tri_x3 = circ_x;
                self.callbacks
                    .draw_triangle(&colour, tri_y1, tri_x1, tri_y2, tri_x2, tri_y3, tri_x3);
            ...
            GemType::RUBY => {
                ...
                self.callbacks
                    .draw_line(&outline_colour, piece_outline_width, y1, x1, y1, x2);
                self.callbacks
                    .draw_line(&outline_colour, piece_outline_width, y1, x1, y2, x1);
                self.callbacks
                    .draw_line(&outline_colour, piece_outline_width, y2, x2, y1, x2);
                self.callbacks
                    .draw_line(&outline_colour, piece_outline_width, y2, x2, y2, x1);
            }
            ...
            GemType::AMETHYST => {
                ...
                self.callbacks
                    .draw_triangle(&colour, yy1, xx1, yy2, xx2, circ_y, circ_x);
                self.callbacks
                    .draw_triangle(&colour, yy2, xx2, yy3, xx3, circ_y, circ_x);
                self.callbacks
                    .draw_triangle(&colour, yy3, xx3, yy4, xx4, circ_y, circ_x);
                self.callbacks
                    .draw_triangle(&colour, yy4, xx4, yy1, xx1, circ_y, circ_x);
                self.callbacks
                    .draw_line(&outline_colour, piece_outline_width, yy1, xx1, yy2, xx2);
                self.callbacks
                    .draw_line(&outline_colour, piece_outline_width, yy2, xx2, yy3, xx3);
                self.callbacks
                    .draw_line(&outline_colour, piece_outline_width, yy3, xx3, yy4, xx4);
                self.callbacks
                    .draw_line(&outline_colour, piece_outline_width, yy4, xx4, yy1, xx1);
            }
            GemType::TOPAZ => {
                let yy1 = y1;
                let xx1 = circ_x;
            ...
            for (x, cell) in row.iter().enumerate() {
                ...
                for adjustment in adjustments {
                    ...
                    if let Some(offset_val) = adjustment.init_offset {
                        offset = Pt {
                            y: offset_val.y as i32,
                            x: offset_val.x as i32,
                        };
                    } else {
                        offset = Pt { y: 0, x: 0 };
                    }
    ...
    pub fn handle_move_updates(&mut self, changes: &GemChanges, prev_state: &State) {
        ...
        */
        ...
        for (change_idx, changes_iter) in changes.changes.iter().enumerate() {
            if change_idx > 0 {
                let mut anim_state = StateAnimation::new(
                    &format!("anim_pause{}", change_idx),
                    changes_iter.prev_state,
                );
                anim_state.add_animation(GemAnimation {
                    src_cell: Pt { y: 1, x: 1 },
            ...
            let mut anim_state2 =
                StateAnimation::new(&format!("anim{}-2", change_idx), changes_iter.prev_state);
            ...
            let mut anim_state3 =
                StateAnimation::new(&format!("anim{}-3", change_idx), changes_iter.new_state);
