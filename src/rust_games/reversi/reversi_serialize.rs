...
pub fn deserialize(serialized_state: &Vec<u8>) -> Result<State, ()> {
	...
	if serialized_state.len() == 264 {
		...
		bitpack::bitunpack(&serialized_state, val_count, max_value, |byte_val: u8, _index: usize| {
		...
		for y in 0..BOARD_SIZE {
			for x in 0..BOARD_SIZE {
				...
				state.board[y][x] = match unpacked_bytes[i] {
					...
					1 => CellState::PLAYER1,
					2 => CellState::PLAYER2,
					_ => { panic!("Unexpected value {} when deserializing at pos {}", unpacked_bytes[i], i); },
				};
			}
		}
		...
		state.last_move = match more_data_byte {
			0b1_000_000 => None,
			_ => Some(Pt { y: (more_data_byte>>3) as i32, x: (more_data_byte & 0b111) as i32 }),
		};
		Ok(state)
	}
}
...
pub fn serialize(state: &State) -> Result<Vec<u8>, ()> {
	...
	let player_turn_serialized = match state_w_metadata.state.player_turn {
		...
		CellState::EMPTY => { panic!("received 'empty' for player turn, invalid"); },
	};
