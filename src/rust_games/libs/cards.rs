...
pub const CARD_SIZE: Pt = Pt { y: CARD_WIDTH*7/5, x: CARD_WIDTH };
...
pub fn card_to_num_opt(card: &Option<Card>) -> u8 {
	if let Some(card) = card { card_to_num(card) } else { 0 }
}
...
fn draw_card_internal(callbacks: &CCallbacksPtr, card: Option<&Card>, face_down: bool, pt: &Pt, highlight: bool) {
	let y1 = pt.y;
	let x1 = pt.x;
	...
	if !face_down && card.is_some() {
		...
		let txt_colour = if suit_is_red(card.suit) { CARD_FG_RED } else { CARD_FG_BLACK };
	
		let padding = 3;
		callbacks.draw_text(&text, &txt_colour, pt.y + padding + TEXT_SIZE_SMALL,
		                    pt.x + padding,
		                    TEXT_SIZE_SMALL,
		                    TextAlign::Left);
		let suit_img_id = suit_to_img_id(card.suit);
		callbacks.draw_graphic(&suit_img_id, y1 + padding + SUIT_SIZE_SMALL.y/2, x2 - padding - SUIT_SIZE_SMALL.x/2,
		                       SUIT_SIZE_SMALL.x, SUIT_SIZE_SMALL.y, None);
		callbacks.draw_text(&text, &txt_colour, y2 - padding - LARGE_TEXT_PADDING, x1 + CARD_SIZE.x/2,
		                    TEXT_SIZE_LARGE,
		                    TextAlign::Middle);
		...
		if highlight {
			...
			callbacks.draw_graphic(IMG_ID_CARD_HIGHLIGHT,
			                       pt.y + CARD_SIZE.y/2, pt.x + CARD_SIZE.x/2,
			                       CARD_SIZE.x + 2*highlight_padding, CARD_SIZE.y + 2*highlight_padding, None);
		}
	} else if face_down {
		callbacks.draw_graphic(IMG_ID_CARD_FACEDOWN,
		                       pt.y + CARD_SIZE.y/2, pt.x + CARD_SIZE.x/2,
		                       CARD_SIZE.x, CARD_SIZE.y, None);
		draw_rect_outline(callbacks, &CARD_OUTLINE, OUTLINE_THICKNESS, y1, x1, y2, x2);
	} else {
...
pub fn new_deck() -> Vec<Card> {
	...
	for rank_num in 1..=13 {
		for suit_num in 0..=3 {
			...
			assert!(num_to_card(card_to_num(&card)).unwrap() == card, "error serializing {:?}", card);
			deck.push(card);
		}
	}
	return deck;
