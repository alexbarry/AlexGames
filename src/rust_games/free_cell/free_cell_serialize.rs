...
fn read_array(serialized_data: &[u8]) -> Option<(&[u8], &[u8])> {
    ...
    if serialized_data.len() < len + 1 {
        panic!(
            "Malformed data, expected {} more bytes, only had {}",
            len + 1,
            serialized_data.len()
        );
    }
...
pub fn deserialize_state(mut serialized_state: &[u8]) -> State {
    ...
    assert!(
        version == FREE_CELL_SERIALIZED_STATE_VERSION,
    ...
    let (cell_ary, mut serialized_state) =
        read_bytes(serialized_state, free_cell_core::BOARD_CELL_COUNT);
    state.cells = cell_ary
        .iter()
        .map(|num: &u8| cards::num_to_card(*num))
        .collect::<Vec<Option<Card>>>()
        .try_into()
        .expect("cell ary not correct len?");

    for goal_idx in 0..free_cell_core::GOAL_COLUMN_COUNT {
        let (goal_col, new_serialized_state) =
            read_array(serialized_state).expect("goal ary missing?");
        serialized_state = new_serialized_state;
        state.goals[goal_idx] = goal_col
            .iter()
            .map(|num: &u8| cards::num_to_card(*num).unwrap())
            .collect();
    }

    for col_idx in 0..free_cell_core::PLAY_AREA_COLUMN_COUNT {
        let (col, new_serialized_state) =
            read_array(serialized_state).expect("play area col ary missing?");
        serialized_state = new_serialized_state;
        state.play_area[col_idx] = col
            .iter()
            .map(|num: &u8| cards::num_to_card(*num).unwrap())
            .collect();
    }
