...
use crate::libs::cards::{
    self, Card, CardDraw, CardDrawSize, Rank, Suit, CARD_SIZE_8_WIDE, DEFAULT_CARD_COLOURS_DARK,
...
impl DrawState {
    ...
    pub fn draw_state(&mut self, callbacks: &'static CCallbacksPtr, mut state: &State) {
        ...
        let new_state = if self.anim_state.is_some() && self.moves_to_anim.len() > 0 {
            ...
            let anim_progress =
                (self.anim_progress_ms as f32) / (self.time_per_move_anim_ms as f32);
            picked_up_card_pos = Some(
                get_pos(src)
                    .add(Pt {
        ...
        for cell_idx in 0..free_cell_core::BOARD_CELL_COUNT {
            ...
            */
            ...
            if let Some(card) = state.cells[cell_idx] {
                ...
                callbacks.draw_text(
                    "FREE",
                    card_space_text_colour,
                    pt.y - 0 * (text_size + text_padding),
                    pt.x,
                    text_size,
                    TextAlign::Middle,
                );
        ...
        for goal_idx in 0..free_cell_core::GOAL_COLUMN_COUNT {
            ...
            */
            ...
            if let Some(card) = state.goals[goal_idx].last() {
                ...
                draw_card.draw_card_space_text(
                    callbacks,
                    &pt,
                    text,
                    card_space_text_colour_goal,
                    32,
                );
            }
        }
...
fn get_pos(card_pos: &CardPos) -> Pt {
    match card_pos {
        CardPos::Cells(cell_idx) => Pt {
            y: padding,
            x: padding + (*cell_idx as i32) * (padding + CARD_SIZE.x),
        },
        CardPos::Goals(goal_idx) => Pt {
            y: padding,
            x: CANVAS_WIDTH
                - (free_cell_core::GOAL_COLUMN_COUNT as i32 - *goal_idx as i32)
                    * (padding + CARD_SIZE.x),
        },
        CardPos::PlayArea(col_idx, card_idx) => Pt {
            y: play_area_start_y + PLAY_AREA_CARD_OFFSET_Y * *card_idx as i32,
