...
use crate::free_cell::free_cell_core::{self, State, CardPos};
use crate::libs::cards::{self, Card, Rank, Suit, draw_card, CARD_SIZE};
...
const play_area_start_x: i32 = CANVAS_WIDTH - (free_cell_core::PLAY_AREA_COLUMN_COUNT as i32) * (CARD_SIZE.x + padding) - padding;
...
pub fn draw_state(callbacks: &'static CCallbacksPtr, mut state: &State, draw_state: &DrawState) {
	...
	for goal_idx in 0..free_cell_core::GOAL_COLUMN_COUNT {
		let pt = Pt {
			y: padding,
			x: CANVAS_WIDTH - (free_cell_core::GOAL_COLUMN_COUNT as i32 - goal_idx as i32) * (padding + CARD_SIZE.x),
		};
		...
		if playable_idx > 0 {
			let overlay_y_end = match draw_state.picked_up_card {
				Some(CardPos::PlayArea(col_idx, _)) if col_idx == i => 
					start_pt.y + PLAY_AREA_CARD_OFFSET_Y * (state.play_area[i].len()-1) as i32 + CARD_SIZE.y,
				_ => start_pt.y + PLAY_AREA_CARD_OFFSET_Y * playable_idx,
			};
			callbacks.draw_rect(&"#00000044", start_pt.y, start_pt.x, 
			                    overlay_y_end,
	...
	if picked_up_cards.len() > 0 {
		...
		let start_pos = start_pos.add(Pt { y: -CARD_SIZE.y/2, x: -CARD_SIZE.x/2 });
		for (card_idx, card) in picked_up_cards.iter().enumerate() {
			let card_idx = card_idx as i32;
			let pos = start_pos.add(Pt { y: card_idx * PLAY_AREA_CARD_OFFSET_Y, x: 0 });
...
pub fn cursor_pos_to_card_pos(state: &State, pos: Pt) -> Option<CardPos> {
	if pos.y < padding + CARD_SIZE.y {
		...
		} else if pos.x > CANVAS_WIDTH - free_cell_core::GOAL_COLUMN_COUNT as i32 * (padding + CARD_SIZE.x) {
			let idx = free_cell_core::GOAL_COLUMN_COUNT as i32 - 1 - (CANVAS_WIDTH as i32 - pos.x) / (CARD_SIZE.x + padding);
