...
-- Helper library to draw shapes from the base APIs defined in alexgames
...
function draw.draw(state)
	...
	--[[
	alexgames.draw_line(BORDER_COLOUR, BORDER_THICKNESS,
	                     0, 0,
	                     0, g_board_width)
	alexgames.draw_line(BORDER_COLOUR, BORDER_THICKNESS,
	                     0,              g_board_width,
	...
	local msg = string.format("time: %s, frame: %5d", alexgames.get_time_of_day(), state.frame_idx)
	...
	alexgames.draw_text('Hello, world!', '#ff0000',
	...
	alexgames.draw_rect(square_color,
	...
	alexgames.draw_circle(circle_color, circle_outline,
	...
	-- a graphic file.
	alexgames.draw_graphic('chess_rook_black',
	...
	elseif state.active_touch ~= nil then
		alexgames.draw_text(string.format('Touch pos: {y=%3.0f, x=%3.0f}', state.user_input_pos_y, state.user_input_pos_x), text_color,
		                     g_board_height - padding, padding,
		                     text_size, alexgames.TEXT_ALIGN_LEFT)
	else
		-- Admittedly I don't have a multi line draw_text function yet
		alexgames.draw_text('Press arrow keys, WASD, or mouse/touch', text_color,
		                     g_board_height - 2*padding - text_size, padding,
		                     text_size, alexgames.TEXT_ALIGN_LEFT)
		alexgames.draw_text('to move ball', text_color,
		                     g_board_height - padding, padding,
		                     text_size, alexgames.TEXT_ALIGN_LEFT)
	end
